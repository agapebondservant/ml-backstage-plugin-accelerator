...
tools:
  ...
  - name: "evidently"
    ...
    console: "https://evidently-ui.default.tanzumlai.com/"
    connector: "http://jupyter-jupyterhub.tanzudatatap.com"
    description: "Dev Instance"
...
//          .catch(error => setFilesPayloadError(error));
   var { value, loading, error } = useAsync(async (): Promise<User[]> => {
   ...
   if (mlFilesPayloadError) {
     ...
     return  (
              <Page themeId="tool">
                ...
                <Content>
                   {[...new Set(mlFilesPayload.tools.filter((mlFile: any) => mlFile.category === props.category).map(mlFile => mlFile.title))].map((mlFileTitle, i: any) =>
                   <>
